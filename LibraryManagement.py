...
class LibraryManagement:
    ...
    def library_management_screen(self):
        ...
        self.student_name_entry = tk.Entry(self.master, font=("Helvetica", 12))
        self.student_name_entry.pack()
        self.student_name_entry.insert(0, "Enter Student Name")
    ...
    def view_books(self):
        available = "\n".join(self.books) if self.books else "None"
        issued = "\n".join([f"{book} → {student}" for book, student in self.lend_list.items()]) if self.lend_list else "None"
        message = f"Available Books:\n{available}\n\nIssued Books:\n{issued}"
